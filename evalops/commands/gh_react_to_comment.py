...
def cleanup_comment_addressed_to_evalops(text: Optional[str]) -> Optional[str]:
    if not text:
        return text
    patterns = [
        r"^\s*evalops\b",
...
def react_to_comment(
    ...
    if issue_ids:
        ...
        api.actions.create_workflow_dispatch(
            workflow_id="evalops-code-review.yml",
            ref=ref,
            inputs={"pr_number": str(pr)},
        )
    else:
        if cfg.answer_github_comments:
            question = cleanup_comment_addressed_to_evalops(comment.body)
    ...
    run = next(
        (
            ...
            if (
                ...
                or "evalops.yml" in r["name"].lower()
            )
