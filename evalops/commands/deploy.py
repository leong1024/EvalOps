...
def deploy(
    ...
    for file_config in workflow_files.values():
        ...
        if file.exists():
            message = f"EvalOps CI workflow already exists at {utils.file_link(file)}"
            if rewrite:
                ui.warning(message)
            else:
    ...
    for key, file_config in workflow_files.items():
        ...
        if file.exists() and "merge_function" in file_config:
            ui.warning("Merging EvalOps CI workflow into existing file:", utils.file_link(file))
    ...
    print(
        mc.ui.green("EvalOps CI workflows have been created.\n"),
