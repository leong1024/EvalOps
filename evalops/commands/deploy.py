...
Usage:
  evalops deploy [--api-type TYPE] [--commit] [--rewrite] [--to-branch NAME]
...
def merge_gitlab_configs(
    ...
    new_include = {"local": ".gitlab/ci/evalops-code-review.yml"}
    include_exists = any(
        (
            "evalops-code-review.yml" in str(item.get("local", ""))
            if isinstance(item, dict)
            else "evalops-code-review.yml" in str(item or "")
        )
...
GIT_PROVIDER_WORKFLOWS = {
    PlatformType.GITHUB: dict(
        code_review=dict(
            path=Path(".github/workflows/evalops-code-review.yml"),
            template="workflows/github/evalops-code-review.yml.j2",
        ),
        react_to_comments=dict(
            path=Path(".github/workflows/evalops-react-to-comments.yml"),
            template="workflows/github/evalops-react-to-comments.yml.j2",
        ),
    ),
    PlatformType.GITLAB: dict(
        code_review=dict(
            path=Path(".gitlab/ci/evalops-code-review.yml"),
            template="workflows/gitlab/evalops-code-review.yml.j2",
        ),
        gitlab_ci=dict(
...
@app.command(
    name="deploy",
    help="\bCreate and deploy EvalOps workflows to your CI pipeline for automatic code reviews."
...
def deploy(
    ...
    to_branch: str = typer.Option(
        default="evalops-ci", help="Branch name for new PR containing EvalOps CI workflows"
    ),
    ...
    """Deploy EvalOps to repository's CI pipeline for automatic code reviews."""
    print(logo())
    ...
    for file_config in workflow_files.values():
        ...
        if file.exists():
            message = f"EvalOps CI workflow already exists at {file_link(file)}"
            if rewrite:
                ui.warning(message)
            else:
    ...
    for key, file_config in workflow_files.items():
        ...
        if file.exists() and "merge_function" in file_config:
            ui.warning(f"Merging EvalOps CI workflow into existing file: {file_link(file)}")
            merge_function = file_config["merge_function"]
            content = merge_function(file, vars=template_vars)
        else:
            template: str = file_config["template"]
            content = render_file(template, **template_vars)
        if not content.endswith("\n"):
            content = content.rstrip() + "\n"
        file.write_text(content, encoding="utf-8")
        created_files.append(file)
    print(
        ui.green("EvalOps CI workflows have been created.\n"),
    ...
    need_to_commit = (
        commit is True
        or commit is None
        and ui.ask_yn(f"Commit & push CI workflows to a {ui.green(to_branch)} branch?")
    )
    is_committed = False
    is_pushed = False
                ...
                if git_platform_type == PlatformType.GITHUB:
                    try:
                        ...
                        res = api.pulls.create(
                            head=to_branch,
                            base=base,
                            title="Add EvalOps CI workflows",
                        )
                        print(f"Pull request #{res.number} created successfully:\n{res.html_url}")
                    except Exception as e:
                        ui.error(f"Failed to create pull request automatically: {e}")
...
def _try_commit_workflow_changes(repo: Repo) -> bool:
    ...
    """
    try:
        repo.git.commit("-m", "Add EvalOps CI workflows")
