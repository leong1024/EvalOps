...
from .llm import LLMContextLengthExceededError, invoke, invoke_parallel
            ...
            if merge_base.hexsha == repo.commit(current_ref or repo.active_branch.name).hexsha:
                ...
                if merge_sha:
                    ...
                    if other_merge_parent:
                        ...
                        # for evalops remote (feature_branch vs origin/main)
...
def file_lines(repo: Repo, file: str, max_tokens: int = None, use_local_files: bool = False) -> str:
    ...
    if max_tokens:
        lines, removed_qty = fit_to_token_size(lines, max_tokens)
        if removed_qty:
    ...
    lines = {
        file_diff.path: (
            file_lines(
                repo,
                file_diff.path,
                config.max_code_tokens - count_tokens(str(file_diff)),
